...
if len(idc.ARGV) != 5:
    LOG.info("[*] [application] [script.json] [Second Script] [MD5] [DATA PATH]")
    idc.qexit(0)
    exit(0)
else:
    path        = idc.ARGV[1]
    secScript   = idc.ARGV[2]
    md5         = idc.ARGV[3]
    data_path   = idc.ARGV[4]


ida_auto.auto_wait()
LOG.info("start il2cpp...")
...
LOG.info('il2cpp Script finished!')
file_globals = runpy.run_path(secScript, {"hash": md5, "path": data_path})
